...
export function apply(ctx: Context,config:Config) {
  ...
  ctx.on('message',async (session) => {
    //提及回复
    const regex = /^neko/i;
    if(regex.test(session.content) && session.content.length < 30){
    ...
    if(session.isDirect){
      ...
      if(!(config.allowPrivateTalkingUsers.includes(session.author.user.id))){
        ...
        historyMessages[session.userId].push(SerializeMessage(session.content))
        console.log(`${formattedDateTime} 私聊${session.userId}收到一条消息 ${session.content}\n距离上一条过去${Date.now() - lastMessageTime}ms`)
        if(Date.now() - lastMessageTime > ctx.config.privateTalkingWaiting){
          let tmp_return = getAIReply(historyMessages[session.userId],apiGPT,singlePrompt,session.userId)
          let reply = tmp_return['reply']
          let emoji = tmp_return['emoji']
          console.log(`${formattedDateTime} 私聊${session.userId}取得回复:${reply.toString()}\nemoji:${emoji}`)
          sendReply(session,reply,emoji,eachLetterCost)
          historyMessages[session.userId] = []
        }
